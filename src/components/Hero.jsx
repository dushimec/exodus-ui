...
import { getUpcomingPostsThunk, selectPosts } from '../slices/postSlice';
...
export default function TravelHero() {
  ...
  useEffect(() => {
    dispatch(getUpcomingPostsThunk());
  }, [dispatch]);

  useEffect(() => {
    if (posts && posts.length > 0) {
      // Transform the posts data into the format needed for the carousel
      const transformedTrips = posts.map(post => ({
        id: post._id,
        title: post.title,
        image: post.photo, // Using the main photo from the post
      }));
  ...
  useEffect(() => {
    console.log("Upcoming Trips:", upcomingTrips); // Log upcoming trips
    console.log("Current Trip Index:", currentTripIndex); // Log current trip index
    AOS.init({ duration: 1500, easing: 'ease-out' });
    AOS.refresh();
  }, [upcomingTrips, currentTripIndex]);
