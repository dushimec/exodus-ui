...
function DestinationCard({ imageSrc, location, description, showTutor, aosAnimation }) {
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden ${aosAnimation ? `aos-animate aos-init" data-aos="${aosAnimation}` : ''}`}>
      <img src={imageSrc} alt={t(location)} className="w-full h-48 object-cover" />
      <div className="p-4">
...
export default function UserProfileDashboard() {
  ...
  const [likedDestinations] = useState([
    {
      imageSrc: "https://cdn.builder.io/api/v1/image/assets/TEMP/33fae0df8bf179790c2123d2aba2911ebe9447d3273945edf5a1e32b89677d6a",
      location: "destinationr.rwanda",
      description: "destinationr.rwandaDescription",
      hasVisited: false
    },
    {
      imageSrc: tukey,
      location: "destinationr.turkey",
      description: "destinationr.turkeyDescription",
      hasVisited: true
    },
    {
      imageSrc: israel,
      location: "destinationr.israel",
      description: "destinationr.israelDescription",
      hasVisited: false
    }
  ]);
  ...
  const renderPersonalInfo = () => (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
      <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4">{t('personalInformation')}</h2>
      <div className="flex flex-col items-center mb-6">
        <div className="relative group">
          <img src={profileImage} alt={t('userAvatar')} className="w-20 h-20 sm:w-24 sm:h-24 rounded-full mb-2" />
          <label htmlFor="avatar-upload" className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
            <Upload className="w-6 h-6 sm:w-8 sm:h-8" />
            <span className="sr-only">{t('uploadNewAvatar')}</span>
          </label>
          <input
            id="avatar-upload"
            type="file"
        ...
        <p className="text-xs sm:text-sm text-primary/60 mt-2">{t('clickToUpload')}</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="fullName" className="block text-primary font-semibold mb-1">{t('fullName')}</label>
          <input id="fullName" type="text" placeholder={t('fullName')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
        <div>
          <label htmlFor="email" className="block text-primary font-semibold mb-1">{t('email')}</label>
          <input id="email" type="email" placeholder={t('email')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
        <div>
          <label htmlFor="phone" className="block text-primary font-semibold mb-1">{t('phoneNumber')}</label>
          <input id="phone" type="tel" placeholder={t('phoneNumber')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
      </div>
      <button className="mt-6 bg-sky-600 text-white px-4 py-2 rounded-md hover:bg-sky-500 transition-colors w-full sm:w-auto">
        {t('saveChanges')}
  ...
  const renderPreferences = () => (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
      <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4">{t('likedDestinations')}</h2>
      <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {likedDestinations.map((destination, index) => (
          <DestinationCard
            key={index}
      ...
      <p className="text-xs sm:text-sm text-primary/80 mt-4">{t('likedDestinationsDescription')}</p>
    </div>
  );

  const renderAccountSettings = () => (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
      <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4">{t('accountSettings')}</h2>
      <div className="space-y-4">
        <div>
          <label htmlFor="currentPassword" className="block text-primary font-semibold mb-1">{t('currentPassword')}</label>
          <input id="currentPassword" type="password" placeholder={t('currentPassword')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
        <div>
          <label htmlFor="newPassword" className="block text-primary font-semibold mb-1">{t('newPassword')}</label>
          <input id="newPassword" type="password" placeholder={t('newPassword')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
        <div>
          <label htmlFor="confirmPassword" className="block text-primary font-semibold mb-1">{t('confirmNewPassword')}</label>
          <input id="confirmPassword" type="password" placeholder={t('confirmNewPassword')} className="w-full p-2 border border-primary/30 rounded-md" />
        </div>
      </div>
      <button className="mt-6 bg-sky-500 text-white px-4 py-2 rounded-md hover:bg-sky-600 transition-colors w-full sm:w-auto">
        {t('changePassword')}
  ...
  const renderSupport = () => (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
      <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4">{t('supportAndContact')}</h2>
      <p className="mb-4 text-sm sm:text-base">{t('supportDescription')}</p>
      <div className="flex items-center mb-2 text-sm sm:text-base">
        <Mail className="text-primary mr-2" size={18} />
        <span>{t('email')}: Oldfoxcoltd@gmail.com</span>
      </div>
      <div className="flex items-center mb-4 text-sm sm:text-base">
        <Phone className="text-primary mr-2" size={18} />
        <span>{t('phone')}: +250788726181</span>
      </div>
      <div className="mb-4">
        <label htmlFor="supportMessage" className="block text-primary font-semibold mb-1">{t('message')}</label>
        <textarea id="supportMessage" placeholder={t('describeIssue')} rows={4} className="w-full p-2 border border-primary/30 rounded-md" />
      </div>
      <button className="mt-4 bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-400 transition-colors w-full sm:w-auto">
        {t('submitRequest')}
      </button>
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 p-4 sm:p-8 max-w-screen-xl mx-auto shadow-lg">
        <div className=" bg-white p-4 mt-14 sm:p-8 rounded-lg shadow-lg">
        <h1 className="text-xl sm:text-2xl font-bold text-black  my-6">{t('userProfileDashboard')}</h1>
          <div className="tabs flex flex-wrap mb-4 font-bold  lg:flex-row flex-col">
            <button
              onClick={() => setActiveTab('personal')}
              className={`tab ${activeTab === 'personal' ? 'text-sky-500  border-b-2 border-sky-500' : 'text-black'} px-3 sm:px-6 py-2 text-md sm:text-base`}
