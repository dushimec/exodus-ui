...
export default function AdminDashboard() {
  ...
  const recentBookings = [
    { name: 'New Delhi To Dhaka', type: 'Oneway', price: '50$', status: 'Pending' },
    // ... other bookings
  ];
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            <a
              ...
              className={`hover:text-blue-300 transition-colors ${activeLink === "Dashboard" ? "text-blue-500" : "text-gray-500"}`}
            ...
            <a
  href="#"
  className={`hover:text-blue-300 transition-colors ${activeLink === "Trips" ? "text-blue-500" : "text-gray-500"}`}
  onClick={() => {
...
>
  Trips
            ...
            <a
              href="#"
              className={`hover:text-blue-300 transition-colors ${activeLink === "Users" ? "text-blue-500" : "text-gray-500"}`}
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <img src={picture} alt="User Avatar" className="h-12 w-12 rounded-full" />
  ...
  className={`p-2 rounded ${
    activeLink === "Dashboard" ? "bg-blue-500 text-white" : "bg-gray-500 text-white"
  }`}
>
  <svg
...
<span
  className={`p-2 rounded ${
    activeLink === "Trips" ? "bg-blue-500 text-white" : "bg-gray-500 text-white"
  }`}
>
  <svg
    xmlns="http://www.w3.org/2000/svg"
  ...
  <span>Add New Trip</span>
            ...
            <li>
              ...
              >
                <span
                  className={`p-2 rounded ${
                    activeLink === "Users" ? "bg-blue-500 text-white" : "bg-gray-500 text-white"
                  }`}
                >
                  {/* Users icon */}
                </span>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {stats.map((stat, index) => (
                  <div key={index} className={`bg-white rounded-lg shadow-md p-6 ${index === 0 ? 'bg-blue-500 text-blue-500' : ''}`}>
                    <h3 className="text-lg font-semibold mb-2">{stat.title}</h3>
                    <p className="text-3xl font-bold mb-2">{stat.value}</p>
                    <p className={`text-sm ${index === 0 ? 'text-blue-200' : 'text-green-500'}`}>{stat.change}</p>
                  </div>
                ))}
              </div>
                  ...
                  <tbody>
                    {recentBookings.map((booking, index) => (
                      <tr key={index} className="border-b last:border-b-0">
                        ...
                        <td className="py-3">
                          <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-sm">{booking.status}</span>
                        </td>
                        <td className="py-3">
                          <button className="text-gray-400 hover:text-gray-600">
                            <MoreVertical size={20} />
