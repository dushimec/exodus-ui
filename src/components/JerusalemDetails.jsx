...
function BookingForm({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-sky-500">{t('israelDetails.bookingForm.title')}</h2>
        <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700">{t('israelDetails.bookingForm.name')}</label>
            <input type="text" id="name" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder={t('israelDetails.bookingForm.name')} />
          </div>
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-gray-700">{t('israelDetails.bookingForm.email')}</label>
            <input type="email" id="email" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder={t('israelDetails.bookingForm.email')} />
          </div>
          <div>
            <label htmlFor="travelers" className="block text-sm font-medium text-gray-700">{t('israelDetails.bookingForm.travelers')}</label>
            <input type="number" id="travelers" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="1" min="1" />
          </div>
          <div>
            <label htmlFor="date" className="block text-sm font-medium text-gray-700">{t('israelDetails.bookingForm.date')}</label>
            <input type="date" id="date" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
          </div>
          <div className="text-xl font-bold text-sky-500 mt-4">
            {t('israelDetails.bookingForm.price')}
          </div>
          <div className="flex gap-4 mt-6">
            <button type="submit" className="flex-1 bg-sky-500 text-white py-2 px-4 rounded-md hover:bg-sky-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-200">{t('israelDetails.bookingForm.bookNow')}</button>
            <button onClick={onClose} className="flex-1 bg-gray-200 text-gray-800 py-2 px-4 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50 transition duration-200">{t('israelDetails.bookingForm.cancel')}</button>
          </div>
        </form>
      </div>
    </div>
...
function RelatedTours() {
  ...
  return (
    <div>
      <div className="text-black text-xl font-bold mb-4">{t('israelDetails.relatedTours.title')}</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {tours.map((tour) => (
          <div key={tour.id} className="bg-white shadow-lg p-4">
            <img
            ...
            <div className="flex justify-between mt-2 text-sm text-gray-500">
              <div className="flex items-center">
                ...
                <span>{tour.likes} {t('israelDetails.relatedTours.likes')}</span>
              </div>
              <span>120$</span>
            </div>

            <div className="mt-4">
              <h4 className="text-black text-sm font-bold">{t('israelDetails.relatedTours.comments')}</h4>
              {tour.comments.length === 0 ? (
                <p className="text-gray-500">{t('israelDetails.relatedTours.noComments')}</p>
              ) : (
                <ul className="text-sm text-gray-700">
                  {tour.comments.map((comment, index) => (
                    <li key={index} className="border-b border-gray-200 py-1">
              ...
              <div className="mt-2">
                <input
                  type="text"
                  className="w-full h-10 border border-gray-300 px-2 mb-2"
                  placeholder={t('israelDetails.relatedTours.addComment')}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && e.target.value.trim()) {
                      addComment(tour.id, e.target.value);
                      e.target.value = "";
...
function JerusalemDetails() {
  ...
  const destinations = [
    {
      name: t('israelDetails.destinations.jerusalem.name'),
      description: t('israelDetails.destinations.jerusalem.description'),
      price: 150,
      date: "January 10, 2025"
    },
    {
      name: t('israelDetails.destinations.bethlehem.name'),
      description: t('israelDetails.destinations.bethlehem.description'),
      price: 100,
      date: "January 10, 2025"
    },
    {
      name: t('israelDetails.destinations.nazareth.name'),
      description: t('israelDetails.destinations.nazareth.description'),
      price: 120,
      date: "January 10, 2025"
    },
    {
      name: t('israelDetails.destinations.seaOfGalilee.name'),
      description: t('israelDetails.destinations.seaOfGalilee.description'),
      price: 90,
      date: "January 10, 2025"
    },
    {
      name: t('israelDetails.destinations.deadSea.name'),
      description: t('israelDetails.destinations.deadSea.description'),
      price: 80,
      date: "January 10, 2025"
    }
  ];
  ...
  return (
    <div className="w-full h-auto bg-stone-50 relative">
      <div className="relative w-full h-[50vh]">
        ...
        <div className="absolute inset-0 bg-black/50" />
        <div className="absolute left-16 top-[70%] transform -translate-y-1/2 text-white text-5xl font-bold">
          {t('israelDetails.mainContent.title')}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-4 lg:flex lg:justify-between gap-8 py-8 px-2 lg:ml-12 ">
        <div className="w-full lg:w-2/3 space-y-8">
          <div>
            <div className="text-sky-500 text-2xl font-bold mb-4">
              {t('israelDetails.mainContent.subtitle')}
            </div>
            <p className="text-black text-lg mb-6">
              {t('israelDetails.mainContent.description')}
            </p>
            <div className="text-sky-500 text-2xl font-bold mb-4">
              {t('israelDetails.mainContent.israelTitle')}
            </div>
            <p className="text-black text-lg">
              {t('israelDetails.mainContent.israelDescription')}
            </p>
          </div>
          ...
          <div>
            <div className="text-black text-xl font-bold mb-4">
              {t('israelDetails.mainContent.recentlyVisited')}
            </div>
            <img
              className="w-full h-auto"
              src={israel}
        ...
        <div className="w-full lg:w-1/3 h-auto mb-8 lg:mb-0 pb-20 mt-10">
          <div className="bg-sky-500 text-white flex justify-between items-center px-6 py-4">
            <div className="text-lg font-semibold">{t('israelDetails.sidebar.totalPrice')}</div>
            <div className="text-3xl font-bold">$540</div>
          </div>

          <div className="bg-white shadow-lg p-6">
            <div className="bg-sky-500 text-white text-center py-3 mb-6">
              <span className="text-base font-semibold">{t('israelDetails.sidebar.tourSummary')}</span>
            </div>
            <div className="space-y-4">
              {destinations.map((destination) => (
                <div key={destination.name} className="flex justify-between">
              ...
              <div className="border-t pt-4 font-bold">
                <div className="flex justify-between">
                  <span>{t('israelDetails.sidebar.total')}</span>
                  <span>$540</span>
                </div>
              </div>
            </div>
            <button 
              ...
              className="w-full mt-6 bg-sky-500 text-white px-4 py-2 rounded-md hover:bg-sky-600 transition duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50"
            >
              {t('israelDetails.bookingForm.bookNow')}
            </button>
          </div>
        </div>
      </div>
